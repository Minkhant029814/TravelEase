...
const reservationData = {
    1: {
        ...
        image: "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1742&q=80",
    },
};
...
const CreateReview = () => {
    ...
    const destinationId = reservations?.destination_id;
    ...
    const fetchDestinations = async () => {
        const response = await api.getDestination(destinationId);
        console.log(response.data);
        setDestinations(await response.data);
    };

    useEffect(() => {
        const fetchData = async () => {
            try {
                const reservationResponse = await api.getReservation(
                    Number(id)
                );
                const reservationData = reservationResponse.data;
                setReservations(reservationData);

                if (reservationData?.destination_id) {
